  ...
  return (
    <div className="border border-zinc-800 rounded-xl px-4 py-3 flex flex-col gap-2">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button onClick={() => flag({ vote: song.vote < 0 ? 0 : -1 })} className={`text-xs px-2 py-1 rounded border ${song.vote < 0 ? "border-amber-500 text-amber-300" : "border-zinc-700 text-zinc-300"}`} title="less like this">{song.vote < 0 ? "↓ ✓" : "↓"}</button>
